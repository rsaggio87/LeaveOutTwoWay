...
% create object files
objstr=' ';
objtocreate = {'vpv','vmv','vpvmv','vvmul','rmvec','ldl_solve','sspmv','spmv','preconditioner'};
for k=1:length(objtocreate)
    filestr = sprintf('..%s..%sSource%sSolver%s%s.c',sep,sep,sep,sep,objtocreate{k});
    mexstr = sprintf('mex -g -largeArrayDims -DSINGLE_PR %s -c %s',include,filestr);
...
for k=1:length(mextocreate)
    mexstr = sprintf('mex -g -largeArrayDims -DSINGLE_PR %s %s.c %s',include,mextocreate{k},objstr);
    eval(mexstr);
end
...
for k=1:length(objtocreate)
    filestr = sprintf('..%s..%sSource%sSolver%s%s.c',sep,sep,sep,sep,objtocreate{k});
    mexstr = sprintf('mex -g -largeArrayDims %s -c %s',include,filestr);
    objstr = sprintf('%s %s.%s',objstr,objtocreate{k},obj);
    eval(mexstr);
end
...
for k=1:length(mextocreate)
    mexstr = sprintf('mex -g -largeArrayDims %s %s.c %s',include,mextocreate{k},objstr);
    eval(mexstr);
end
